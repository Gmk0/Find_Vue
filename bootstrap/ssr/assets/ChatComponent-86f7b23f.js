...
import { ssrRenderAttrs, ssrRenderSlot, ssrRenderClass, ssrRenderAttr, ssrRenderComponent } from "vue/server-renderer";
import { u as useLayoutStore } from "./store-9e809cfe.js";
import { _ as _sfc_main$4 } from "./OtherPanelUser-0912d387.js";
import "pinia";
import "axios";
import "@vueuse/core";
import "vue3-popper";
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      var _a, _b;
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: ["flex flex-col w-full mt-0 bg-red-400 messages main-content h-100vh chat-app", isShowChatInfo.value && "lg:mr-80"]
      }, _attrs))}><div class="chat-header h-[61px] border-b border-slate-150 dark:border-navy-700 relative z-10 flex w-full shrink-0 items-center justify-between bg-white px-[calc(var(--margin-x)-.5rem)] shadow-sm transition-[padding,width] duration-[.25s] dark:bg-navy-800"><div class="flex items-center space-x-5"><div class="ml-1 h-7 w-7"><button class="${ssrRenderClass([unref(layoutStore).isSidebarExpanded && "active", "menu-toggle ml-0.5 flex h-7 w-7 flex-col justify-center space-y-1.5 text-primary outline-none focus:outline-none dark:text-accent-light/80"])}"><span></span><span></span><span></span></button></div><div x-show="activeChat?.name" class="flex items-center space-x-4 cursor-pointer font-inter"><template x-if="activeChat?.profile_path !== null"><div class="avatar"><img class="rounded-full"${ssrRenderAttr("src", "/storage/" + ((_a = _ctx.activeChat) == null ? void 0 : _a.profile_path))} alt="avatar"></div></template><template x-else><div class="avatar"><img class="rounded-full"${ssrRenderAttr("src", (_b = _ctx.activeChat) == null ? void 0 : _b.profile_url)} alt="avatar"></div></template><div><p class="font-medium text-slate-700 line-clamp-1 dark:text-navy-100" x-text="activeChat?.name"></p><p x-text="activeChat?.last_time" class="mt-0.5 text-xs"></p></div></div><div x-show="!activeChat?.name" class="flex items-center space-x-4 cursor-pointer font-inter"><div><p class="font-medium text-slate-700 line-clamp-1 dark:text-navy-100"> Conversation </p></div></div></div><div class="flex items-center -mr-1"><button class="hidden p-0 rounded-full btn2 h-9 w-9 text-slate-500 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:text-navy-200 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25"><svg xmlns="http://www.w3.org/2000/svg" class="h-5.5 w-5.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></button><button class="${ssrRenderClass([isShowChatInfo.value ? "text-primary dark:text-accent-light" : "text-slate-500 dark:text-navy-200", "p-0 rounded-full btn2 h-9 w-9 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25 sm:flex"])}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5.5 w-5.5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9.25 21.167h5.5c4.584 0 6.417-1.834 6.417-6.417v-5.5c0-4.583-1.834-6.417-6.417-6.417h-5.5c-4.583 0-6.417 1.834-6.417 6.417v5.5c0 4.583 1.834 6.417 6.417 6.417ZM13.834 2.833v18.334"></path></svg></button></div></div><div class="grow overflow-y-auto bg-green-400 main-messages px-[calc(var(--margin-x)-.5rem)] py-5 transition-all duration-[.25s]"></div></div>`);
    };
  }
};
