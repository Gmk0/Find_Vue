...
const _sfc_main = /* @__PURE__ */ Object.assign({
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(_attrs)}><div class="min-h-screen px-4 py-20 bg-gray-100 dark:bg-gray-900 md:px-8"><div class="mx-2 mb-4"><nav class="flex" aria-label="Breadcrumb"><ol class="inline-flex items-center space-x-1 md:space-x-3"><li class="inline-flex items-center">`);
      ...
      ssrRenderList(__props.categories, (category) => {
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<img src="/storage/illustration/business.svg" class="w-20 h-20 rounded-md" alt=""${_scopeId}><div class="flex flex-col gap-2"${_scopeId}><h4 class="mt-3 mb-1 md:text-[20px] text-[16px] dark:text-white font-semibold text-slate-600 duration-200 group-hover:text-white"${_scopeId}>${ssrInterpolate(category.name)}</h4><div${_scopeId}><span${_scopeId}></span></div></div>`);
            } else {
              return [
                createVNode("img", {
                ...
                createVNode("div", { class: "flex flex-col gap-2" }, [
                  createVNode("h4", { class: "mt-3 mb-1 md:text-[20px] text-[16px] dark:text-white font-semibold text-slate-600 duration-200 group-hover:text-white" }, toDisplayString(category.name), 1),
      ...
      _push(`<!--]--></div></div><div class="min-h-screen px-4 pt-8 bg-white dark:bg-gray-800 md:px-8"><div class="flex justify-start"><h1 class="text-2xl font-semibold text-black dark:text-white"> Parcourir toutes les catégories </h1></div><!--[-->`);
      ssrRenderList(__props.categoriesAll, (category) => {
        ...
        _push(`</div><hr><div class="grid grid-cols-2 gap-4 mt-4 md:grid-cols-3 lg:grid-cols-4"><!--[-->`);
        ssrRenderList(category.sub_categories, (sub) => {
          _push(`<div class="mb-4">`);
          _push(ssrRenderComponent(unref(Link), {
            href: _ctx.route("SubcategoryName", [category.name, sub.name]),
            class: "px-4 text-base font-semibold text-gray-600 dark:text-gray-200"
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
