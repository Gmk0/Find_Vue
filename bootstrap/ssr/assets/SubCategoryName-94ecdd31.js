import { ref, watch, resolveComponent, mergeProps, unref, withCtx, createVNode, openBlock, createBlock, toDisplayString, Fragment, renderList, createCommentVNode, useSSRContext } from "vue";
...
const _sfc_main = /* @__PURE__ */ Object.assign({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative w-full min-h-screen py-16 pb-12" }, _attrs))}><div><div class="relative flex items-center justify-between h-16 px-8 lg:h-20 dark:bg-gray-600 bg-skin-fill"><div class="flex items-center justify-center"><h1 class="text-lg font-bold text-white lg:text-4xl">${ssrInterpolate(props.subcategory.name)}</h1></div><div class="flex">`);
      _push(ssrRenderComponent(unref(Link), {
        href: _ctx.route("createProject"),
        class: "relative lg:flex items-center hidden justify-center h-10 px-4 mx-auto text-sm duration-300 rounded-md bg-gray-50 before:absolute before:inset-0 before:transition hover:scale-105 active:duration-75 active:scale-95 sm:w-max"
      }, {
      ...
      _push(`</div></div><div class="px-4 mt-4"><nav class="flex" aria-label="Breadcrumb"><ol class="inline-flex items-center space-x-1 md:space-x-3"><li class="inline-flex hidden items-center"><a href="#" onclick="history.back()" class="inline-flex items-center px-4 text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 15l-3-3m0 0l3-3m-3 3h8M3 12a9 9 0 1118 0 9 9 0 01-18 0z"></path></svg><span class="mr-2"> Retour </span></a></li><li aria-current="page">`);
      _push(ssrRenderComponent(unref(Link), {
        href: _ctx.route("categoryName", props.category.name),
        class: "flex items-center"
      }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<svg class="w-3 hidden h-3 mx-1 text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10"${_scopeId}><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"${_scopeId}></path></svg><span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400"${_scopeId}>${ssrInterpolate(props.category.name)}</span>`);
          } else {
            return [
              (openBlock(), createBlock("svg", {
                class: "w-3 hidden h-3 mx-1 text-gray-400",
                "aria-hidden": "true",
              ...
              createVNode("span", { class: "ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400" }, toDisplayString(props.category.name), 1)
            ];
          }
        }),
        _: 1
      ...
      _push(`</div><div class="grid sticky top-0 col-span-12 gap-4 lg:grid-cols-12 lg:gap-2"></div><div class="flex flex-row justify-between gap-2 px-4 lg:col-span-3"><div class=""><div class="block lg:hidden"><button class="border py-1.5 px-4 rounded-md"> filtre </button></div></div><div class="block m-2 lg:hidden">`);
