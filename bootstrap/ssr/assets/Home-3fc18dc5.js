...
import { _ as _sfc_main$2 } from "./WebLayout-017f2aea.js";
import { Swiper, SwiperSlide } from "swiper/vue";
import { f as useStore, h as useMissions } from "./userInfo-17d621bd.js";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.categories, (category) => {
        _push(`<div><a href="" class="flex flex-col items-center px-2 py-4 duration-200 bg-white border shadow-lg cursor-pointer group rounded-xl border-amber-500/10 shadow-amber-300/10 hover:bg-amber-600">`);
        if (category.illustration = true) {
          _push(`<img${ssrRenderAttr("src", "/storage/" + category.illustration)} class="w-20 h-20 rounded-md" alt="">`);
        } else {
          _push(`<!---->`);
        }
        _push(`<h4 class="mt-3 mb-1 md:text-[20px] text-[16px] font-semibold text-slate-600 duration-200 group-hover:text-white">${ssrInterpolate(category.name)}</h4></a></div>`);
      });
      _push(`<!--]--></div></div></div>`);
