import { ref, computed, watch, resolveComponent, mergeProps, withCtx, openBlock, createBlock, createVNode, createTextVNode, toDisplayString, useSSRContext } from "vue";
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrInterpolate, ssrRenderStyle } from "vue/server-renderer";
import { _ as _sfc_main$1 } from "./FreelanceLayout-58ff1bab.js";
import pickBy from "lodash/pickBy.js";
import throttle from "lodash/throttle.js";
import { router } from "@inertiajs/vue3";
import { a as useCategoryStore } from "./userInfo-b7520541.js";
import "./MainSidebarF-9442bf81.js";
import "vue3-popper";
import "@vueuse/core";
import "pinia";
import "axios";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(missions.value, (mission) => {
        ...
        _push(ssrRenderComponent(_component_Link, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`${ssrInterpolate(mission.title)}`);
            } else {
              return [
        ...
        _push(`</div><p class="mt-1 line-clamp-3">`);
        if (!showMore.value) {
        ...
        _push(`</div><span class="line-clamp-1">${ssrInterpolate(mission.user.name)}</span></a><div class="self-stretch w-px mx-3 my-1 bg-slate-200 dark:bg-navy-500"></div><span class="shrink-0 text-slate-400 dark:text-navy-300">${ssrInterpolate(mission.created_at)}</span></div></div><div class="flex justify-between gap-4 mt-1"><div class="flex items-center"><h1 class="text-lg font-bold text-amber-600">${ssrInterpolate(mission.budget)} $</h1></div>`);
