    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.categories, (category) => {
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<img src="/storage/illustration/business.svg" class="w-12 rounded-md lg:w-20 lg:h-20" alt=""${_scopeId}><div class="flex flex-col gap-2"${_scopeId}><h4 class="mt-3 mb-1 md:text-[20px] text-sm dark:text-white block font-semibold text-slate-600 duration-200 group-hover:text-white"${_scopeId}>${ssrInterpolate(category.name)}</h4><div${_scopeId}><span${_scopeId}></span></div></div>`);
            } else {
              return [
                createVNode("img", {
