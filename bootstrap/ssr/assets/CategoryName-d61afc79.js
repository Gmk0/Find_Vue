import { ref, watch, resolveComponent, mergeProps, unref, withCtx, openBlock, createBlock, createVNode, Fragment, renderList, toDisplayString, useSSRContext } from "vue";
...
import { _ as _sfc_main$3 } from "./ServiceCard-ff571472.js";
...
const _sfc_main = /* @__PURE__ */ Object.assign({
  ...
  setup(__props) {
    ...
    watch(form, throttle(() => {
      router.get(
        route("categoryName", props.category.name),
        pickBy(form.value),
        {
          preserveState: true,
          preserveScroll: true,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative w-full min-h-screen py-16 pb-12" }, _attrs))}><div><div class="relative h-24 bg-skin-fill dark:bg-gray-600"><img class="hidden object-cover w-full h-full opacity-70" src="" alt="image" title=""><div class="absolute inset-0 flex items-center justify-center"><h1 class="text-lg font-bold text-white lg:text-4xl">${ssrInterpolate(props.category.name)}</h1></div></div><div class="px-4 mt-4"><nav class="flex" aria-label="Breadcrumb"><ol class="inline-flex items-center space-x-1 md:space-x-3"><li class="inline-flex items-center"><a href="#" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white"><svg class="w-3 h-3 mr-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20"><path d="m19.707 9.293-2-2-7-7a1 1 0 0 0-1.414 0l-7 7-2 2a1 1 0 0 0 1.414 1.414L2 10.414V18a2 2 0 0 0 2 2h3a1 1 0 0 0 1-1v-4a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v4a1 1 0 0 0 1 1h3a2 2 0 0 0 2-2v-7.586l.293.293a1 1 0 0 0 1.414-1.414Z"></path></svg> Acceuil </a></li><li aria-current="page">`);
      ...
      _push(`</li><li aria-current="page"><div class="flex items-center"><svg class="w-3 h-3 mx-1 text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"></path></svg><span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">${ssrInterpolate(props.category.name)}</span></div></li></ol></nav></div><div class="relative mt-4"><div class="sticky top-0 z-30 grid h-auto grid-cols-12 px-4 py-2 bg-white dark:bg-gray-800 lg:z-0 lg:bg-transparent lg:relative"><div class="lg:col-span-3"></div><div class="grid col-span-12 gap-4 lg:col-span-9 lg:grid-cols-12 lg:gap-2"><div class="px-4 lg:col-span-9">`);
      _push(ssrRenderComponent(_sfc_main$2, {
        modelValue: form.value.search,
        "onUpdate:modelValue": ($event) => form.value.search = $event,
        class: "!rounded-full w-full !shadow-md",
        placeholder: "recherche",
        icon: "search"
      }, null, _parent));
      _push(`</div><div class="flex flex-row justify-between gap-2 px-4 lg:col-span-3"><div class=""><div class="block lg:hidden"><button class="border py-1.5 px-4 rounded-md"> filtre </button></div></div><div class="flex gap-2 lg:hidden"><div>`);
      ...
      _push(`</div></div></div></div></div><div class="grid grid-cols-12 px-4"><div class="relative col-span-3 p-2"><div class="${ssrRenderClass([showFiltre.value ? "fixed inset-0    top-0  bottom-0  dark:bg-gray-800 bg-white z-[250] p-4 transition-all duration-200 w-full" : "hidden w-full mt-0   z-20", "overflow-x-hidden overflow-y-auto rounded-md lg:h-auto lg:block"])}"><div class="flex flex-col p-2"><h1 class="mb-2 text-lg font-bold text-gray-800">Sous category</h1><div class="flex flex-wrap gap-2 lg:grid lg:grid-cols-1"><!--[-->`);
      ssrRenderList(__props.subcategories, (subcategory) => {
        _push(`<div><button class="${ssrRenderClass([{ "border-2 border-amber-500 bg-amber-100 text-amber-700 translate-x-4": form.value.sub_categorie == subcategory.name, "bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200": form.value.sub_categorie != subcategory.name }, "flex gap-2 p-2 transition-all transform rounded-lg shadow-sm bg-gray-50 hover:translate-x-4"])}"><img src="" class="object-fill w-8 p-1 rounded-md" alt=""><span class="text-gray-700 dark:text-gray-50">${ssrInterpolate(subcategory.name)}</span><span class="ml-2 bg-white text-gray-800 px-1 py-0.5 text-[10px] rounded-full">${ssrInterpolate(subcategory.service_count)}</span></button></div>`);
      });
      _push(`<!--]--></div><div class="mt-4"><h1 class="mb-4 text-lg font-bold text-gray-800">Prix</h1><div class="w-full space-y-1 dark:text-gray-100"><div class="flex flex-col gap-2 p-2"></div><div aria-hidden="true" class="flex justify-between px-1"><div class="flex justify-between gap-4 p-2 border">`);
      _push(ssrRenderComponent(_sfc_main$2, {
        value: "10 $",
        disabled: "",
      ...
      ssrRenderList(__props.categories, (category) => {
        ...
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<img${ssrRenderAttr("src", "/storage/" + category.illustration)} class="object-fill w-8 p-1 rounded-md" alt=""${_scopeId}><span class=""${_scopeId}>${ssrInterpolate(category.name)}</span><div class="absolute inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold text-white bg-red-500 border-2 border-white rounded-full -top-2 -right-2 dark:border-gray-900"${_scopeId}>${ssrInterpolate(category.servcies_counts)}</div>`);
            } else {
              return [
                createVNode("img", {
                ...
                createVNode("div", { class: "absolute inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold text-white bg-red-500 border-2 border-white rounded-full -top-2 -right-2 dark:border-gray-900" }, toDisplayString(category.servcies_counts), 1)
              ];
            }
          }),
          _: 2
