<div class="relative w-full">
    ...
    <div class="flex-1 lg:p-2 p-6 justify-between flex flex-col h-screen border border-solid border-blue-500 fixed {{ $winPosition=="left"?"left-0":"right-0" }} bottom-0 bg-white dark:bg-dark-900 shadow z-30 {{ $panelHidden ? 'hidden' : '' }}" style="{{ $winWidth }}" id="chat-window">
            ...
            <div class="relative flex items-center space-x-4">
                ...
                <div class="flex flex-col leading-tight">
                    <div class="flex items-center mt-1 text-2xl">
                        <span class="mr-3 text-gray-700">ChatGPT</span>
                    </div>
                    <span class="text-lg text-gray-600"></span>
                </div>
            </div>
            <div class="flex items-center space-x-2">
                <button type="button" class="inline-flex items-center justify-center w-8 h-8 text-gray-500 transition duration-500 ease-in-out border rounded-lg hover:bg-gray-300 focus:outline-none" wire:click="resetSession()" title="clear text [CTRL+D]">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                      </svg>
                </button>
                <button type="button" class="inline-flex items-center justify-center w-8 h-8 text-gray-500 transition duration-500 ease-in-out border rounded-lg hover:bg-gray-300 focus:outline-none" wire:click="changeWinWidth()" title="resize [CTRL+R]" style="margin-left:0.25rem;">
                    ...
                    <div class="chat-message">
                        <div class="flex items-end">
                            <div class="flex flex-col items-start order-2 mx-2 space-y-2 text-xs">
                                <div><div class="block px-4 py-2 text-gray-600 bg-gray-300 rounded-lg rounded-bl-none ">@isset($message['content']){!! \Illuminate\Mail\Markdown::parse($message['content']) !!}@endisset</div></div>
                            </div>
                            <div class="relative flex items-center justify-center w-5 h-5 p-1 text-white rounded-full" style="background-color: rgb(16, 163, 127);">
                                <x-filament-chatgpt-bot::chatgpt-svg />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="chat-message">
                        <div class="flex items-end justify-end">
                            <div class="flex flex-col items-end order-1 max-w-xs mx-2 space-y-2 text-xs">
                                <div><div class="block px-4 py-2 text-white bg-blue-600 rounded-lg rounded-br-none">{!! \Illuminate\Mail\Markdown::parse($message['content']) !!}</div></div>
                            </div>
                            @php
                                $name = auth()->user()? substr(auth()->user()->name,0,1):"匿";
                                $avatar = "https://ui-avatars.com/api/?name={$name}&color=FFFFFF&background=111827'";
                            @endphp
                            <img src="{{ $avatar }}" alt="My profile" class="order-2 w-6 h-6 rounded-full">
                        </div>
                    </div>
                @endif
                @endif
        ...
        <div class="px-4 pt-4 mb-2 border-t-2 border-gray-200 sm:mb-0">
            ...
            <div class="relative flex flex-col flex-grow w-full py-2 bg-gray-200 rounded-md shadow md:py-3 md:pl-4 dark:border-gray-900/50 dark:text-white dark:bg-gray-700">
                <textarea wire:model.defer="question" tabindex="0" data-id="root" style="max-height: 200px; height: 24px; " placeholder="Send a message..." class="w-full p-0 pl-2 m-0 bg-transparent border-0 resize-none pr-7 focus:ring-0 focus:outline-none focus:placeholder-gray-400 dark:bg-transparent md:pl-0" id="chat-input"></textarea>
